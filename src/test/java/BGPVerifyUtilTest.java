...
public class BGPVerifyUtilTest {
  ...
  public void checkTriplePatternGroup() {
    ...
    Map<CompositeKey, List<ResultValue>>
        nodeResults =
        new HashMap<CompositeKey, List<ResultValue>>();
  ...
  public void checkAdjacentConnections() {
    ...
    assertFalse(BGPVerifyUtil.checkAdjacentConnections(nodeKey,
                                                       nodeResults, parentResults, childResults,
                                                       bgp, "Hans"));
  ...
  public void getShortcuts() {
    ...
    assertEquals(expected, BGPVerifyUtil.getShortcuts(bgp,
                                                      new CompositeKey(
                                                          tp11.getStringRepresentation(),
                                                          Position.SUBJECT)));

    // Test object
    expected.clear();
    expected.add(new Shortcut(new CompositeKey(tp31
                                                   .getStringRepresentation(), Position.OBJECT),
                              new CompositeKey(
                                  tp11.getStringRepresentation(), Position.SUBJECT)));

    assertEquals(expected, BGPVerifyUtil.getShortcuts(bgp,
                                                      new CompositeKey(
                                                          tp21.getStringRepresentation(),
                                                          Position.OBJECT)));
