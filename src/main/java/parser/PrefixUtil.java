 ...
 */
public final class PrefixUtil implements Serializable {
    ...
    if (input.startsWith("<") && input.endsWith(">")) {
      for (int i = 0; i < prefixes.length; i++) {
        if (input.contains(prefixes[i][1])) {
          ...
          String tmpInput = input.replace(prefixes[i][1], prefixes[i][0]);
          // remove closing bracket of URI
          return tmpInput.substring(0, input.length() - 1);
        }
      }
    }
    return input;
