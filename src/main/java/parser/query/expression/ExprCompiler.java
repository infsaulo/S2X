package parser.query.expression;
...
import parser.query.expression.op.Add;
import parser.query.expression.op.Bound;
import parser.query.expression.op.Equals;
import parser.query.expression.op.AbstractExpr2;
import parser.query.expression.op.GreaterThan;
import parser.query.expression.op.GreaterThenEqual;
import parser.query.expression.op.IExpression;
import parser.query.expression.op.Lang;
import parser.query.expression.op.LangMatches;
import parser.query.expression.op.LessThan;
import parser.query.expression.op.LessThanEqual;
import parser.query.expression.op.LogAnd;
import parser.query.expression.op.LogOr;
import parser.query.expression.op.Not;
import parser.query.expression.op.NotEquals;
import parser.query.expression.op.RegEx;
import parser.query.expression.op.Subtract;
...
public class ExprCompiler implements ExprVisitor {
	...
	if (nv.asNode().isLiteral()) {
	    stack.push(new parser.query.expression.op.NodeValue(
		    nv.asQuotedString()));
	} else {
	    stack.push(new parser.query.expression.op.NodeValue(
	...
	stack.push(new parser.query.expression.op.ExprVar(
		nv.getVarName()));
    }
	...
	if (func instanceof E_Regex) {
	    ...
	    AbstractExpr2 operator = new RegEx(left, right);
	    stack.push(operator);
