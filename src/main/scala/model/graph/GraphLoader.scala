...
import model.graph.node.{VertexAttribute, VertexInterface}
import org.apache.hadoop.conf.Configuration
import org.apache.hadoop.fs.FileSystem
import org.apache.spark.SparkContext
import org.apache.spark.graphx.{Edge, Graph, PartitionStrategy, VertexId}
import org.apache.spark.rdd.RDD
import parser.rdf.{LineMalformedException, RDFParser}
...
object GraphLoader {
  ...
  private var LOGGER: Logger = Logger.getLogger(GraphLoader.getClass.getSimpleName)
  ...
  def loadGraphCountBased(graphPath: String, context: SparkContext) {
    ...
    val splitted: RDD[(String, String, String)] = lines
      .filter(line => line.length > 0 && !line.startsWith("@prefix") && !line.startsWith("@PREFIX"))
      // split the line
    ...
    val replacedObjects: RDD[(String, (Long, String))] = joinedTriples
      .map(line => (line._2._1._1, (line._2._2, line._2._1._2)));
    ...
    if (Const.varPred) {
      ...
      val replacedSubjects: RDD[(String, (Long, Long))] = joinedSubjects
        .map(line => (line._2._1._2, (line._2._2, line._2._1._1)));

      // Join the nodes for the predicate
      val predicateNodes: RDD[((Long, Long, String), Option[Long])] = replacedSubjects
        .leftOuterJoin(zippedNodes)
      ...
      edges = joinedSubjects
        .map(line => Edge(line._2._2, line._2._1._1, new EdgeAttribute(line._2._1._2)))
    }
  ...
  def loadGraphHashBased(graphPath: String, context: SparkContext) {
    ...
    val hashedNodes: RDD[(VertexId, VertexInterface)] = distinctNodes
      .map(node => (hash(node), new VertexAttribute(node)))
    ...
    if (Const.varPred) {
      ...
      val predicatesToJoin: RDD[(String, (Long, Long))] = splitted
        .map(triple => (triple._2, (hash(triple._1), hash(triple._3))))
      val nodesToJoin: RDD[(String, VertexId)] = hashedNodes
        .map(node => (node._2.asInstanceOf[VertexAttribute].getAttribute(), node._1))
      ...
      edges = splitted
        .map(triple => Edge(hash(triple._1), hash(triple._3), new EdgeAttribute(triple._2)))
    }

    splitted.unpersist(true)
    _originalGraph = Graph(hashedNodes, edges).persist(Const.STORAGE_LEVEL)
      .partitionBy(PartitionStrategy.EdgePartition1D)
    ...
    if (hashedCount.compareTo(distHashedCount) != 0) {
      ...
      LOGGER.log(Level.SEVERE,
                 "Collisions occured while hashing. The graph can not be loaded with hashing. Use the option -countBased to load the graph without hashing.")
    }
  }
  ...
  def loadPreloadedGraph(graphPath: String, context: SparkContext) {
    ...
    val nodes: RDD[(VertexId, VertexInterface)] = context
      .objectFile(fs.getHomeDirectory().toString() +
