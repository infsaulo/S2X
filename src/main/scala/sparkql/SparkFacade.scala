...
import model.graph.GraphLoader
...
import sparql.operator.bgp._
import sparql.operator.filter.Filter
import sparql.operator.optional.LeftJoin
import sparql.operator.order.Order
import sparql.operator.projection.Projection
import sparql.operator.result.ResultBuilder
...
/**
  ...
  */
object SparkFacade extends BasicGraphPattern with ResultBuilder with Projection with LeftJoin
                           with Filter with Order {
  ...
  def createSparkContext() {
    ...
    conf
      .set("spark.kryo.registrator", "de.tf.uni.freiburg.sparkrdf.sparql.serialization.Registrator")
  ...
  /**
    ...
    */
  def buildResult(triples: java.util.List[TriplePattern],
                  res: RDD[(VertexId, VertexInterface)]): RDD[SolutionMapping] = {
    // Broadcast all Sparql triples to all workers
    val broadcastedTriples: Broadcast[java.util.List[TriplePattern]] = context.broadcast(triples)
  ...
  /**
    * Build the results out of the graph from the graph model
    */
  def projectResults(vars: java.util.Set[String],
                     res: RDD[SolutionMapping]): RDD[SolutionMapping] = {
    // Broadcast all Sparql triples to all workers
  ...
  /**
    * Execute a left join
    */
  def optional(joinVars: java.util.List[String], left: RDD[SolutionMapping],
