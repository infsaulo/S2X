package sparql.operator.bgp

import model.graph.edge.EdgeAttribute
import model.graph.node._
import model.rdf.result._
import model.rdf.triple._
import org.apache.spark.broadcast.Broadcast
import org.apache.spark.graphx._
import org.apache.spark.rdd._
import sparql.message._
import sparql.message.datatype._
...
object TriplePatternProcessor {
    ...
    graph.triplets.flatMap(triplet => {
      ...
      triples.value.foreach(queryTriple => {
        ...
        if (queryTriple.isFulfilledByTriplet(subjectNode, edge.getEdgeAttribute(), objectNode)) {
           ...
           */
          val subResultValue: ResultValue = new ResultValue(edge.getEdgeAttribute(),
                                                            objectNode.getAttribute());
          val subkey: CompositeKey = new CompositeKey(queryTriple.getStringRepresentation(),
                                                      Position.SUBJECT)
          ...
          if (!TriplePatternUtils.isObjectLiteral(triples.value, queryTriple.getObject())) {
            val objResultValue: ResultValue = new ResultValue(edge.getEdgeAttribute(),
                                                              subjectNode.getAttribute());
            val objKey: CompositeKey = new CompositeKey(queryTriple.getStringRepresentation(),
                                                        Position.OBJECT)
            ...
            val toChild: AdjacentNodeMessage = new AdjacentNodeMessage(
              new AdjacentNode(triplet.srcId, subjectNode.getAttribute(), true))
            val toParent: AdjacentNodeMessage = new AdjacentNodeMessage(
              new AdjacentNode(triplet.dstId, objectNode.getAttribute(), false))
          ...
          if (queryTriple.isPredicateVariable() && edge.hasNodeId()) {
             ...
             */
            val predResultValue: ResultValue = new ResultValue(subjectNode.getAttribute(),
                                                               objectNode.getAttribute());
            val predKey: CompositeKey = new CompositeKey(queryTriple.getStringRepresentation(),
                                                         Position.PREDICATE)
            val predicateResult: NodeResultMessage = new NodeResultMessage(predKey,
                                                                           predResultValue);
             ...
             */
            val toChild2 = new AdjacentNodeMessage(
              new AdjacentNode(triplet.srcId, subjectNode.getAttribute(), true))
            val toParent2 = new AdjacentNodeMessage(
              new AdjacentNode(triplet.dstId, objectNode.getAttribute(), false))
            result += ((id, toChild2))
            result += ((id, toParent2))
             ...
             */
            val toChild = new AdjacentNodeMessage(
              new AdjacentNode(id, edge.getEdgeAttribute(), true))
            val toParent = new AdjacentNodeMessage(
              new AdjacentNode(id, edge.getEdgeAttribute(), false))
            result += ((triplet.dstId, toChild))
            result += ((triplet.srcId, toParent))
          }
        }
  ...
  /**
    ...
    *
    */
  def joinResultsIntoGraph(msgs: RDD[(VertexId, IMessage)],
                           graph: Graph[VertexInterface, EdgeAttribute]): Graph[VertexInterface, EdgeAttribute] = {
    ...
    val newVerts: RDD[(VertexId, VertexInterface)] = graph.vertices
      .innerJoin(results)((vertexId, node, message) => {
        /*
  ...
  /**
    ...
    *
    */
  def joinNeighborsIntoGraph(msgs: RDD[(VertexId, IMessage)],
                             graph: Graph[VertexInterface, EdgeAttribute]): Graph[VertexInterface, EdgeAttribute] = {
