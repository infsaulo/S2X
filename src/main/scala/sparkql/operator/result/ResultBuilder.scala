...
trait ResultBuilder {
  ...
  /**
    ...
    */
  protected def buildResult(triples: Broadcast[java.util.List[TriplePattern]],
                            vertices: RDD[(VertexId, VertexInterface)]): RDD[SolutionMapping] = {
    val toSearch: java.util.Set[TriplePattern] = TriplePatternUtils
      .getTriplePatternsForResult(triples.value);

    /*
     * Step 1: Build the initial solution mappings out of the results from the graph vertices
     */
    val vertexResults: RDD[SolutionMapping] = InitialSolutionMappingBuilder
      .findVertexResultNodes(toSearch, vertices).persist(StorageLevel.MEMORY_AND_DISK)
     ...
     */
    val allPartialResults = SolutionMappingBuilder
      .joinSolutionMappings(partialResults, vertexResults)
