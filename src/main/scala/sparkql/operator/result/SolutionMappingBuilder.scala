...
import sparql.operator.result.util.{SolutionMapping, _}
...
object SolutionMappingBuilder {
    ...
    partialResults.map(partialResult => {
      ...
      partialResult.getAllTriples().foreach(triplePattern => {
        ...
        if (firstRound) {
          ...
          solutionMappingForPartialResult = vertexResults.filter(
            result => result.getInitialTriplePattern()
              .equals(triplePattern.getStringRepresentation()))
          firstRound = false
          ...
          val isEmpty = solutionMappingForPartialResult
            .mapPartitions(iter => Iterator(!iter.hasNext), true).reduce(_ && _)
          ...
          if (!isEmpty) {
            ...
            val toJoin = vertexResults.filter(result => result.getInitialTriplePattern()
              .equals(triplePattern.getStringRepresentation()))
            ...
            val joinVariables = TriplePatternUtils
              .getJoinVariables(firstSolutionMapping.getStoredVariables(), triplePattern)
           ...
           */
            ...
            solutionMappingForPartialResult = join.map(joined => {
              val sol: SolutionMapping = new SolutionMapping(
                joined._2._1.getInitialTriplePattern());
  ...
  /**
    ...
    */
  private def getJoinKey(joinVariables: java.util.List[String],
                         solMapping: SolutionMapping): String = {
    var joinKey: String = ""
